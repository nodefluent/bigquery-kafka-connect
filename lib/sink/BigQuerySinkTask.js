    ...
    runBatch() {
        ...
        return new Promise((resolve, reject) => this.table.insert(
            ...
            (error, apiResponse) => {
                ...
                if (error) {
                    // re-add the rows to the buffer, but remove the last buffer element
                    // because kafka-connect will resend it
                ...
                rows.forEach(row => this.parentConfig.emit("model-upsert", row.insertId));
